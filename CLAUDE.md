...
- **Game marketing: `./.claude/workflows/game-marketing-workflow.md`** (Mobile game top charts)
...
- `game-market-analyst` - Mobile game top charts & market intelligence
...
**Game Marketing** (`.claude/skills/game-marketing/`)
- `topchart-analyzer` - Modular skill for top chart analysis with reusable components
  - `lib/sensortower-api-client.js` - SensorTower API wrapper with batching
  - `lib/ranking-calculator.js` - Pure logic for position changes & insights
  - `lib/report-generator.js` - Markdown report formatting
  - `lib/category-mappings.js` - iOS/Android category/genre mappings
  - See: `topchart-analyzer.md` for full documentation
  - Example: `example-usage.js` for complete workflow
...
|--------|----------|---------|
| `sensortower` | App Intel | iOS/Android app analytics, ASO |
...
### Game Marketing
- `/game:topchart` - Analyze mobile game top charts (month-over-month) [MCP + Modular]
